...
import 'package:bh_finder/Screen/Home/reserve.rerent.dart';
...
class RerentRoom extends StatefulWidget {
  final String? token;

  const RerentRoom({super.key, this.token});

  @override
  State<RerentRoom> createState() => _RerentRoomState();
}

class _RerentRoomState extends State<RerentRoom> {
  ...
  void initState() {
    ...
    super.initState();
    bHouseRoom =
        FirebaseFirestore.instance.collection('Rooms').doc(Get.arguments[0].toString()).get();
  }
  ...
  Future<List<String>> _loadImage() async {
    ListResult result =
    await storage.ref().child("roomImages/${Get.arguments[0].toString()}").listAll();
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                EasyDateTimeLine(
                                  initialDate: DateTime.now().add(Duration(days: 1)), // Set date to next 3 days
                                  onDateChange: (newDate) {
                                    setState(() {
                                      selectedDateCheckInRerent = newDate;
                                    });
                                    print('$selectedDateCheckInRerent');
                                  },
                                ),
                                SizedBox(height: 20),
                              ],
                                    ...
                                    try {
                                      ...
                                      if (querySnapshot.docs.isNotEmpty) {
                                        ...
                                        if (currentUser == null ||
                                          ...
                                          Get.to(()=>RerentRoomReserve(), arguments: [Get.arguments[0].toString(),]);
                                          // Navigator.push(
                                          //     context,
                                          //     MaterialPageRoute(
                                          //       builder: (context) =>
