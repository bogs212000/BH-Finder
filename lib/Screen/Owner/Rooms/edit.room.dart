...
class _EditRoomScreenState extends State<EditRoomScreen> {
  ...
  late TextEditingController _roomPrice;
  late TextEditingController _roomDescriptions;
  ...
  void initState() {
    ...
    _roomName = TextEditingController(text: widget.roomName);
    _roomPrice = TextEditingController(text: widget.roomPrice);
    _roomDescriptions = TextEditingController(text: widget.roomDescriptions);

    // Add listeners to detect changes
    _roomName.addListener(_checkIfChanged);
    _roomPrice.addListener(_checkIfChanged);
    _roomDescriptions.addListener(_checkIfChanged);
  }
  ...
  Future<List<String>> _loadImages() async {
    ListResult result =
        await storage.ref().child("roomImages/${widget.roomId}").listAll();
  ...
  void dispose() {
    ...
    _roomName.dispose();
    _roomPrice.dispose();
    _roomDescriptions.dispose();
    super.dispose();
  }

  void _checkIfChanged() {
    // Check if any text field value differs from the initial values
    if (_roomName.text != (widget.roomName ?? "") ||
        _roomPrice.text != (widget.roomPrice ?? "") ||
        _roomDescriptions.text != (widget.roomDescriptions ?? "")) {
      setState(() {
        _isChanged = true;
      });
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> _openImagePicker() async {
      ...
      if (pickedImage != null) {
        setState(() {
          _roomImage = File(pickedImage.path);
        });
        // setState(() {
        //   loading = false;
        // });
                                          ...
                                          Container(
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              image: DecorationImage(
                                                image: CachedNetworkImageProvider(images[index]), // Set the image from the network
                                                    ...
                                                    GestureDetector( onTap: () async {
                                                      QuickAlert.show(
                                                        ...
                                                        onConfirmBtnTap: () async {
                                                          await _deleteImage(images[index]); // Delete the image
                                                          setState(() {
                              ...
                              ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    try {
                                      ...
                                      final filePath =
                                          "${appDocDir.path}/path/to/${_roomName.text}.jpg";

                                      // Get the selected image file (use the path from the selected image)
                                      final file = File(_roomImage!.path);
                                      ...
                                      final uploadTask = storageRef
                                          .child(
                                              "roomImages/${widget.roomId.toString()}/${DateTime.now().toString()}.jpg")
                              ...
                              QuickAlert.show(
                                ...
                                onConfirmBtnTap: () async {
                                  ...
                                  await FirebaseFirestore.instance
                                      .collection('Rooms')
                                      .doc(widget.roomId.toString()) // Assuming the document is based on user email
                                      .update({
                                    'roomNameNumber': _roomName.text,
                                    'price': _roomPrice.text,
                                    'descriptions': _roomDescriptions.text,
